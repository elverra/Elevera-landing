...
export function registerRoutes(app: Express): void {
    ...
    try {
      ...
      const sectors = [
        ...
        },
        {
          id: "2",
          name: "Technology",
          description: "Electronics, gadgets, and tech services",
          is_active: true,
        },
        {
          id: "3",
        ...
        },
        {
          ...
          description: "Medical services, fitness, and wellness",
          is_active: true,
        },
        {
          id: "6",
          name: "Education",
          description: "Courses, books, and educational services",
          is_active: true,
        },
        {
          id: "7",
        ...
        },
        {
          id: "9",
          name: "Automotive",
          description: "Car services, parts, and transportation",
          is_active: true,
        },
        {
          id: "10",
          name: "Professional Services",
          description: "Business services, consulting, and legal",
          is_active: true,
        },
      ];
  ...
  app.get("/api/discounts/featured", async (req, res) => {
    try {
      ...
      const featuredDiscounts = merchants
        .filter(
          (merchant) => merchant.isActive === true && merchant.featured === true
        )
        .map((merchant) => {
          const merchantSector = sectors.find(
            (s) => s.id === merchant.sectorId
          );
          const location = `${merchant.city}, ${merchant.country}`;
          const imageUrl =
            merchant.logoUrl || getImageByBusinessType(merchant.businessType);
          ...
          return {
            ...
            description:
              ...
              getDiscountDescription(
                ...
                Number(merchant.discountPercentage)
              ),
            location: location,
            image_url: imageUrl,
  ...
  const getDiscountDescription = (
    ...
    return (
      descriptionMap[businessType] ||
      `Special ${discount}% discount for members`
    );
  };
  ...
  const getImageByBusinessType = (businessType: string): string => {
    const imageMap: Record<string, string> = {
      ...
      Entertainment:
        "https://images.unsplash.com/photo-1489599558431-d5c5a1a4d8b7?w=400",
      Automotive:
    ...
    try {
      ...
      if (location && location !== "all") {
        ...
        allDiscounts = allDiscounts.filter((d) =>
          d.location.includes(locationStr)
        );
      }
      ...
      const discounts = [
        {
          ...
          description:
            "15% off all coffee and pastries. Perfect for your morning boost!",
          merchantId: "m1",
          ...
          isActive: true,
          termsAndConditions:
            "Valid for dine-in and takeaway. One per customer per day.",
          imageUrl:
        ...
        },
        {
          ...
          description:
            "30% off on smartphones, laptops, and accessories for premium members.",
          merchantId: "m2",
          ...
          isActive: true,
          termsAndConditions:
            "Valid on purchases above 100,000 CFA. Warranty included.",
          imageUrl:
            "https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=600",
          createdAt: "2025-01-01T00:00:00Z",
    ...
    try {
      ...
      if (!title || !merchantId || !sectorId || !discountPercentage) {
        return res
          ...
          .json({
            error:
              "Title, merchant, sector, and discount percentage are required",
          });
      }
    ...
    try {
      ...
      if (referrerId) {
        ...
        try {
          ...
          console.log(
            `✅ Affiliate reward processed for referrer ${referrerId}`
          );
        } catch (rewardError) {
  ...
  app.get("/api/affiliate-rewards/:referrerId", async (req, res) => {
    try {
      const rewards = await storage.getAffiliateRewardsByReferrer(
  ...
  app.get("/api/affiliate-dashboard/:userId", async (req, res) => {
    try {
      ...
      const totalCreditPoints = rewards
        ...
        .reduce(
          (sum, r) =>
            sum + parseFloat(r.creditPointsAwarded?.toString() || "0"),
          0
        );
      const totalCommissions = rewards
        .filter((r) => r.rewardType === "commission")
        .reduce(
          (sum, r) => sum + parseFloat(r.commissionAmount?.toString() || "0"),
          0
        );
        ...
        referrals.map(async (referral) => {
          ...
          return {
            ...
            earnings: relatedReward
              ? parseFloat(
                  relatedReward.creditPointsAwarded?.toString() || "0"
                ) +
                parseFloat(relatedReward.commissionAmount?.toString() || "0")
              : 0,
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          ${
            mode === "demo"
              ? "<p><small>This is a demo transaction. Contact Orange operator to activate production credentials.</small></p>"
              : ""
          }
  ...
  app.post("/api/payments/initiate-orange-money", async (req, res) => {
    try {
      ...
      if (!tokenResponse.ok) {
        ...
        console.error(
          "Failed to get Orange Money token:",
        ...
        if (
          ...
          console.warn(
            "Orange Money production credentials issue. Using demo mode."
          );
      ...
      const paymentRequest = {
        ...
        notif_url: `${req.protocol}://${req.get(
          "host"
      ...
      const paymentResponse = await fetch(
        `${orangeConfig.baseUrl}/webpayment`,
        {
          method: "POST",
          headers: {
      ...
      if (!paymentResponse.ok) {
        ...
        console.error(
          "Failed to create Orange Money payment:",
        ...
        if (paymentResponse.status === 403 || paymentResponse.status === 401) {
          console.warn(
            "Orange Money payment creation failed. Using demo mode."
          );
  ...
  app.get("/api/test/sama-money", async (req, res) => {
    try {
      ...
      const samaConfig = {
        baseUrl:
          process.env.SAMA_BASE_URL ||
          "https://smarchandamatest.sama.money/V1/",
      ...
      if (!samaConfig.merchantCode)
        missingCredentials.push("SAMA_MERCHANT_CODE");
      if (!samaConfig.publicKey) missingCredentials.push("SAMA_PUBLIC_KEY");
      if (!samaConfig.transactionKey)
        missingCredentials.push("SAMA_TRANSACTION_KEY");
  ...
  app.get("/demo-payment", (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .payment-card { border: 2px solid ${
              provider === "sama" ? "#10b981" : "#ff6600"
            }; border-radius: 10px; padding: 30px; text-align: center; }
            .amount { font-size: 2em; color: ${
              provider === "sama" ? "#10b981" : "#ff6600"
            }; font-weight: bold; margin: 20px 0; }
            .btn { background: ${
              provider === "sama" ? "#10b981" : "#ff6600"
            }; color: white; border: none; padding: 15px 30px; border-radius: 5px; font-size: 1.1em; cursor: pointer; margin: 10px; }
            .btn:hover { background: ${
              provider === "sama" ? "#059669" : "#e55a00"
            }; }
    ...
    <body>
        <div class="payment-card">
            <h1>${provider === "sama" ? "💰" : "🍊"} ${String(
      provider
    ).toUpperCase()} Money Payment</h1>
            <div class="demo-notice">
                <strong>Demo Mode:</strong> This is a simulated payment for testing purposes
            </div>
            <div class="amount">${amount} ${currency}</div>
            <p><strong>Reference:</strong> ${reference}</p>
            <p>Proceed with your payment using ${String(
              provider
            ).toUpperCase()} Money</p>
  ...
  app.post("/api/payments/initiate-sama-money", async (req, res) => {
    try {
      ...
      if (!merchantCode || !publicKey || !transactionKey || !userId) {
        console.warn(
          "SAMA Money credentials not configured - service unavailable"
        );
      ...
      try {
        ...
        console.warn(
          "SAMA Money API unavailable, using demo mode fallback:",
          fetchError?.message
        );

        // Fallback to demo mode when API is unavailable
        const demoReference = `SAMA_DEMO_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`;
        const demoPaymentUrl = `/demo-payment?provider=sama&amount=${amount}&currency=${currency}&reference=${demoReference}&phone=${encodeURIComponent(
          phone
        ...
        responseData = {
          ...
          status: "initiated",
          message:
            "Payment redirected to demo mode - SAMA Money API temporarily unavailable",
        };
      }
    ...
    try {
      const categories = [
        ...
        },
        {
          id: "5",
          name: "Books & Media",
          description: "Books, music, and other media",
        },
        {
          id: "6",
          name: "Automotive",
  ...
  app.get("/api/memberships/:userId", async (req, res) => {
    try {
      ...
      const membership = {
        ...
        is_active:
          user.membershipTier === "premium" || user.membershipTier === "elite",
      ...
      res
        ...
        .json({
          error:
            error instanceof Error
              ? error.message
              : "Failed to fetch merchant applications",
        });
    }
  });
  ...
  app.put("/api/admin/merchants/:id", async (req, res) => {
    try {
      ...
      if (updateData.discount_percentage !== undefined)
        merchant.discountPercentage = Number(updateData.discount_percentage);
      ...
      if (updateData.logo_url !== undefined)
        merchant.logoUrl = updateData.logo_url;
      if (updateData.rating !== undefined)
        merchant.rating = Number(updateData.rating);
      if (updateData.is_active !== undefined)
        merchant.isActive = updateData.is_active;
      if (updateData.featured !== undefined)
        merchant.featured = updateData.featured;
  ...
  app.delete("/api/admin/payment-gateways/:id", async (req, res) => {
    try {
      ...
      if (hasActiveTransactions) {
        return res.status(400).json({
          error:
            "Cannot delete gateway with active transactions. Please wait for all transactions to complete.",
        });
      }
  ...
  app.post("/api/admin/payment-gateways/:id/test", async (req, res) => {
    try {
      ...
      console.log(
        `Testing payment gateway ${id} with amount: ${amount} ${currency}`
      );
      ...
      switch (id) {
        ...
        case "sama_money":
          ...
          const hasSamaCredentials =
            process.env.SAMA_MERCHANT_CODE && process.env.SAMA_PUBLIC_KEY;
          testResult = {
